...
class TestBuildAcceptanceContext:
    ...
    def test_context_contract_values(
        self, sample_contract, sample_expert, sample_activities
    ):
        ...
        assert (
            context["order_id"]
            == "HDCG-2024-001/2024/HDCG-TEST-PROJECT-Additional test info"
        )
        ...
        assert context["acceptance_name"] == (
            "BIÊN BẢN BÀN GIAO VÀ NGHIỆM THU LẦN 1 HỢP ĐỒNG THUÊ KHOÁN CÔNG VIỆC"
        )
        assert context["dd"] == 1
        assert context["mm"] == 6
    ...
    def test_context_expert_values(
        self, sample_contract, sample_expert, sample_activities
    ):
    ...
    def test_context_activities_count(
        self, sample_contract, sample_expert, sample_activities
    ):
    ...
    def test_context_activity_fields(
        self, sample_contract, sample_expert, sample_activities
    ):
    ...
    def test_context_activity_stt_starts_from_1(
        self, sample_contract, sample_expert, sample_activities
    ):
    ...
    def test_context_amounts_are_strings(
        self, sample_contract, sample_expert, sample_activities
    ):
    ...
    def test_context_totals_calculation_single_activity(
        self, sample_contract, sample_expert, sample_activities
    ):
        ...
        context = service._build_acceptance_context(
            sample_contract, sample_expert, single_activity
        )
    ...
    def test_context_totals_calculation_multiple_activities(
        self, sample_contract, sample_expert, sample_activities
    ):
    ...
    def test_context_null_fields_default_to_empty_string(
        self, sample_contract, sample_expert, sample_activities
    ):
        ...
        service = WordExportService()
        context = service._build_acceptance_context(
            contract_no_optional, sample_expert, sample_activities
        )
    ...
    def test_context_additional_information_has_prefix(
        self, sample_contract, sample_expert, sample_activities
    ):
        ...
        context = service._build_acceptance_context(
            ...
            sample_expert,
            sample_activities,
            acceptance_additional_information="Additional test info",
        )
        assert context["additional_information"] == "- Additional test info"

    def test_context_activity_amounts_use_dot_separator(
        self, sample_contract, sample_expert, sample_activities
    ):
    ...
    def test_context_activity_supports_fractional_working_days(
        self, sample_contract, sample_expert, sample_activities
    ):
    ...
    def test_context_expert_null_fields_default_to_empty_string(
        self, sample_contract, sample_expert, sample_activities
    ):
        ...
        service = WordExportService()
        context = service._build_acceptance_context(
            sample_contract, expert_minimal, sample_activities
        )
    ...
    def test_context_end_date_format(
        self, sample_contract, sample_expert, sample_activities
    ):
...
class TestExportAcceptanceReport:
    ...
    def test_export_with_single_activity(
        self, sample_contract, sample_expert, sample_activities
    ):
    ...
    def test_export_calls_docxtpl_correctly(
        self, mock_docxtpl, sample_contract, sample_expert, sample_activities
    ):
    ...
    @patch("app.services.word_export.DocxTemplate")
    def test_export_context_has_correct_activity_count(
        self, mock_docxtpl, sample_contract, sample_expert, sample_activities
    ):
...
class TestBuildContextComparison:
    ...
    def test_both_contexts_have_same_structure(
        self, sample_contract, sample_expert, sample_activities
    ):
        """Both context builders should produce consistent key structures."""
        service = WordExportService()

        contract_context = service._build_context(
            sample_contract, sample_expert, sample_activities
        )
        acceptance_context = service._build_acceptance_context(
            sample_contract, sample_expert, sample_activities
        )
        ...
        assert len(contract_context["activities"]) == len(
            acceptance_context["activities"]
        )
    ...
    def test_acceptance_context_has_different_totals(
        self, sample_contract, sample_expert, sample_activities
    ):
        ...
        assert acceptance_context[
            "total_amount"
        ] == f"{expected_acceptance_total:,.0f}".replace(",", ".")

    def test_build_context_additional_information_has_prefix(
        self, sample_contract, sample_expert, sample_activities
    ):
